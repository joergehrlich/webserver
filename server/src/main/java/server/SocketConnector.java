...
public class SocketConnector implements Connector 
{
  ...
  private class Acceptor implements Runnable {
    ...
    /**
     * Constructor.
     * @param index An index attached to the acceptor, for identification.
     */
    ...
    public void run() {
      try {
        while(true) {
          ...
          server.handle(new SocketConnection(socket));
          
          LOG.info(this + "Finised dispacthing connection from: " + socket.getRemoteSocketAddress().toString());
        }
      } catch (Exception e) {
   ...
   */
  private Set<SocketConnection> connections;
  ...
  /**
   * Constructor. 
   * @param acceptors Number of acceptor threads.
   */
  public SocketConnector(int acceptors) {
    this.acceptors = acceptors;
    connections = new HashSet<SocketConnection>();
  }
